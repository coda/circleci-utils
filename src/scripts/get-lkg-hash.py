...
def parse_args():
    ...
    parser.add_argument(
        '--repo-name', help='CircleCI repository name', default=os.environ.get('CIRCLE_PROJECT_REPONAME'))
    parser.add_argument('--branch-name', help='GitHub branch name', default='master')
    parser.add_argument('--token', help='CircleCI API token', default=os.environ.get('CIRCLE_LOCK_API_TOKEN'))
    parser.add_argument('--job-name', default='deploy-head', help='Find the LKG build for this job name')
